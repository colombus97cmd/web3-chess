...
function App() {
  ...
  const tabStyle = (tab) => ({
    ...
    color: activeTab === tab ? "#000" : "#888", fontWeight: "bold"
  });
  return (
    <div className="App">
      <header className="App-header">
        ...
        <div style={{ display: "flex", gap: "4px", marginTop: "24px", justifyContent: "center" }}>
          ...
          <button style={tabStyle("real")} onClick={() => setActiveTab("real")}>Mode Real</button>
        </div>
        <div style={{ border: "2px solid", borderRadius: "0 12px 12px 12px", padding: "28px", background: "#1a1d23", minWidth: "min(560px, 90vw)" }}>
          {activeTab === "trial" && <TrialMode onConnectWallet={() => setActiveTab("real")} />}
          {activeTab === "real" && (
            <div>
              <div style={{ textAlign: "center", marginBottom: "20px" }}><w3m-button /></div>
              {!isConnected && <p>Connectez votre wallet.</p>}
              {isConnected && (
                <div>
                  <div className="game-controls">
                    <input type="text" placeholder="ID" value={gameId} onChange={e => setGameId(e.target.value)} />
